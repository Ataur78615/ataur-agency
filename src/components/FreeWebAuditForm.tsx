...
export default function FreeWebAuditForm() {
  ...
  if (submitted) {
    return (
      ...
      >
        ...
        <h2 className="text-4xl font-black text-gray-900 dark:text-white mb-4 italic">Submission Successful!</h2>
        <p className="text-gray-500 dark:text-gray-400 text-lg mb-10 font-medium">
          Our experts will analyze your website and get back to you shortly.
        </p>
        ...
        <div className="p-8 bg-blue-50 dark:bg-blue-900/20 rounded-[2rem] border border-blue-100 dark:border-blue-800">
          <h3 className="text-xl font-bold text-gray-800 dark:text-gray-200 mb-6 flex items-center justify-center gap-2">
             <MessageCircle className="text-blue-600" /> Connect with Admin
          </h3>
          <a 
            href={`https://wa.me/${process.env.NEXT_PUBLIC_WHATSAPP_NUMBER?.replace(/\D/g, '') || '917250570798'}`} 
            target="_blank" 
    ...
    <section className="py-20 relative overflow-hidden">
      ...
      <div className="max-w-4xl mx-auto px-4">
        <div className="text-center mb-12">
          ...
          >
            Unlock your website's true potential with a professional technical and performance review.
          </motion.p>
        </div>
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="space-y-3">
              ...
              <div className="relative group">
                <Globe className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 w-5 h-5 group-focus-within:text-blue-600 transition-colors" />
                <input 
                  required
                  type="text"
          ...
          <button 
            type="submit"
            disabled={loading}
            className="w-full py-5 bg-gradient-to-r from-blue-600 to-indigo-700 text-white rounded-2xl font-black text-lg shadow-xl shadow-blue-600/20 flex items-center justify-center gap-3 hover:scale-[1.02] active:scale-95 transition-all disabled:opacity-50"
          >
            {loading ? (
              <div className="w-6 h-6 border-4 border-white/30 border-t-white rounded-full animate-spin" />
            ) : (
              <>
                <Send className="w-5 h-5" /> Submit Audit Inquiry
              </>
            )}
          </button>

          <div className="pt-8 border-t border-gray-100 dark:border-gray-800 flex flex-col items-center">
            <h3 className="text-lg font-bold text-gray-500 mb-6 italic">Connect with Admin</h3>
            <a 
              href={`https://wa.me/${process.env.NEXT_PUBLIC_WHATSAPP_NUMBER?.replace(/\D/g, '') || '917250570798'}`} 
              target="_blank" 
